...
add_line_after (Document * doc, int paragraph_number, int line_number,
  ...
  if (doc != NULL && paragraph_number <= MAX_PARAGRAPHS && new_line != NULL
...
remove_line (Document * doc, int paragraph_number, int line_number) {
  ...
  if (doc != NULL && paragraph_number <= MAX_PARAGRAPHS && line_number < nl) {
...
replace_text (Document * doc, const char *target, const char *replacement){
  ...
  /*pointer that will move for copying points, starts at the beggining of temp */
  char *check = &temp[0];
